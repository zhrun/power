      ...
      <el-scrollbar class="scorll" style="height:100%; width:100%">
        <el-form :model="expressForm" ref="expressForm" :rules="expressRule" label-width="100px" class="form-content">
          <div class="inline-form">
            ...
            <el-form-item label="来电时间" prop="happenTime" class="right-inline">
              <el-date-picker v-model="expressForm.happenTime" type="datetime" format="yyyy-MM-dd HH:mm"  placeholder="来电时间" :disabled="allSetReady">
              </el-date-picker>
            </el-form-item>
          </div>  
          ...
          <el-form-item label="来电标题" prop="title">
...
import { queryzbPageList,getDutyHandleDetails,postDutyHandle,putDutyHandle,getGenerateNo } from "@/views/index/api/onDuty/api.ps.js";
export default {
  components: { visBreadcrumb },
  data() {
    return {
      breadcrumbData: [{ name: "文电拟办" },{ name: "来电拟办", routerName: "callCommunication"}, { name: "新增来电拟办" }],
      expressForm:{
        uid:"",
        code:0,
        happenTime:"",
  ...
  mounted() {
    ...
    if(this.$route.query.fromTxt=="add"){
      ...
      this.expressForm.dutyWorkMattersInfo.mattersSourceType=62
      this.breadcrumbData=[{ name: "文电拟办" },{ name: "来电拟办", routerName: "callCommunication"}, { name: "编辑来电拟办" }]
  ...
  methods: {
    ...
    closeFullScreen(){
      ...
      this.$router.push({ name: "callCommunication", query:{refresh:true} })
    },
    //获取编号
    getGenerateNo(){
      let that = this
    ...
    backPre(){
      ...
      that.$alert('是否确定要返回？', '返回', {
        ...
        callback: action => {
          if(action==='confirm'){
            this.$router.push({ name: "callCommunication" })
          }
          if(action==='cancel'){
            return
          }
