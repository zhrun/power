        ...
        <el-scrollbar class="main_right">
          <el-form ref="userForm" :model="model" :rules="rules" label-width="110px">
            <table width="100%">
              <tr>
                <td class="form_item">
                  <el-form-item label="上级主管单位" prop="parentUnitName" ref="parentUnitName" :required="!isEdit">
              ...
              <tr>
                <td class="form_item">
                  <el-form-item label="工作所在区域" prop="workArea" ref="workArea" :required="!isEdit">
              ...
              <tr>
                <td class="form_item">
                  <el-form-item label="行政区划代码" prop="areaCode" ref="areaCode" :required="!isEdit">
                    <span v-if="isEdit">{{model.areaCode}}</span>
                    <div class="input_btn" v-else>
                      <el-input v-model="model.areaCode" disabled></el-input>
                    </div>
                  </el-form-item>
                </td>
                <td class="remarks" v-html="unitConfig['areaCode'][0]"></td>
                <td class="remarks" v-html="unitConfig['areaCode'][1]"></td>
              </tr>
              <tr>
                <td class="form_item">
                  <el-form-item label="单位名称" prop="unitName" ref="unitName">
                    <div class="input_btn">
              ...
              <tr>
                <td class="form_item">
                  <el-form-item label="单位简称" prop="unitShortName" ref="unitShortName">
                    <div class="input_btn">
              ...
              <tr>
                <td class="form_item">
                  <el-form-item label="单位电话" ref="unitPhone">
                    <div
                      class="phone_item"
                      v-for="(item, index) in model.unitPhone"
                      :class="{ 'is-error': phoneValidate['unitPhone'][index] }"
              ...
              <tr>
                <td class="form_item">
                  <el-form-item label="值班室电话" ref="dutyPhone">
                    <div
                      class="phone_item"
                      v-for="(item, index) in model.dutyPhone"
                      :class="{ 'is-error': phoneValidate['dutyPhone'][index] }"
...
import { unitRemarkConfig } from "./unitRemarkConfig";
import {
  checkNameExt,
  validatePhone,
  checkZipCode,
...
export default {
  ...
  data() {
    ...
    const parentUnitName = (rule, value, callback) => {
      if (this.isEdit) {
        return callback();
      }
      if (!value) {
        return callback("请选择上级主管单位");
      }
      callback();
    };
    const workArea = (rule, value, callback) => {
      if (this.isEdit) {
        return callback();
      }
      if (!value) {
        return callback("请选择工作所在区域");
      }
      callback();
    };
    ...
    return {
      ...
      rules: {
        parentUnitName: [{ validator: parentUnitName, trigger: "blur" }],
        workArea: [{ validator: workArea, trigger: "change" }],
  ...
  methods: {
    ...
    getPhoneErrorKey() {
      ...
      this.phoneKeyList.some((key) => {
        // 任一个电话号码规则校验不通过，抛出报错的字段名
        if (this.phoneValidate[key].some(Boolean)) {
          errorKey = key;
          return true;
        }
      });
    ...
    buildParams() {
      ...
      // 组装电话号码
      params.unitType = "1";
      params.phoneList = this.buildPhoneParams();
      // 删除不需要的参数
      delete params.unitPhone;
      delete params.dutyPhone;
      delete params.fax;
    ...
    initEditData(data) {
      ...
      this.model.idCardType = data.idCardType + "";
      this.querySortList();
    },
    // 校验通过后请求接口
