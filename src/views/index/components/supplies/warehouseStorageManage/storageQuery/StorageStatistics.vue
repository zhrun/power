    ...
    <div class="filter_container">
      <div style="width:100%">
        ...
        <el-table :data="tableData" style="width:100%" v-loading="loading">
          ...
          <el-table-column prop="storage" label="库存总数" width="">
						<template slot-scope="{row}">
							{{row.storage || 0}}
						</template>
          </el-table-column>
          <el-table-column prop="availableStorage" label="可用库存" width="">
						<template slot-scope="{row}">
...
export default {
	...
	methods: {
		...
		// 构建请求参数
		buildParams() {
			const { unitUid, keyword, pageNum, pageSize } = this.searchForm
			const params = {
