...
import { queryUsersList,checkUserRename, getJoblist,  createUsersList,   updateUsersList,  queryCurWarehouseList ,resetPwd } from "@/views/index/api/supplies/api.zr.js";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      breadcrumbData: [{ name: "基础信息" }, { name: "仓库管理员" }],
      orgTableData: [],
      formInline: {
        name:'',
        keyword:''
  ...
  //重置密码
   // 重置密码请求
    ...
    async addUser() {
      const {code} = await createUsersList({
