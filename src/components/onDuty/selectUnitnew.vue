        ...
        <div class="unit-left">
          <div class="com-content">            
            <el-scrollbar class="scorll" style="height:100%; width:100%">
              ...
              <div class="com-left-unit" v-for="(item, index) in allList" :key="index+999">
...
export default {
  ...
  mounted(){
    this.allList = JSON.parse(JSON.stringify(this.unitList))
  ...
  methods:{
    selectAll(v){
      if(v){
        this.allList.map((obj)=>{
          obj.unitChecked=true
        })
        this.selectList = JSON.parse(JSON.stringify(this.allList))
      }else{
        this.allList.map((obj)=>{
    ...
    delUnit(item, i){
      let oi=-1
      this.allList.map((obj, index)=>{
        if(obj.uid==item.uid){
          oi=index
        }
      })
      this.selectList.splice(i, 1)
      this.allList[oi].unitChecked=false
      this.allChecked=false
    },
    callFunc(){
